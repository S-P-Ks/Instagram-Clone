...
function NavigationBar({ hasUserLoggedIn }: { hasUserLoggedIn?: boolean }) {
    const [cookie, setcookie] = useState()
    const [c, setCookie] = useCookies(["cookieToken"])
    const router = useRouter();
    const user = useUserStore((s: userStore) => s.user);

    useEffect(() => {
        setcookie(c.cookieToken);
    }, [])
    ...
    return (
        <div className='m-8 lg:flex'>
            <div>
                ...
                <div className='flex w-40 justify-between mb-4 hover:bg-slate-400 p-2 rounded-full hover:text-black'>
                    <div className='w-6 rounded-full bg-cyan-400'>{user.name.at(0)}</div>
                    <div>Profile</div>
                </div>
                {(cookie == null || hasUserLoggedIn) && <Link href={"/signin"} className='flex w-40 justify-between mb-4 hover:bg-slate-400 p-2 rounded-full hover:text-black'>
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
    return {
        props: {
            hasUserLoggedIn: context.req.cookies['cookieToken'] != null
        }
    }
}